...
mtl_ufp_dat <- read_csv("data/monitoring_data/montreal_2020_annual_with_land_use_sharp2023.csv")
...
rm(min_days_dat)
# notice I added \n to some of the titles and names. At the "\n" in ggplot adds a linebreak. I put them there to give a little bit of spacing below the titles. The proper way to do so would be with "theme(plot.title = ....)", but I find using "\n" useful when doing quick exploratory plots because then I don't have to look up exactly how to move titles around.
...
# calculate the associations (r2) between UFP and each of the highly correlated variables. account for meteorological conditions by including them in the lm()
hi_corr_pairs_r2 <- 
  mtl_ufp_dat_clean_std %>%
...
# sometimes r sessions in the cloud will crash, it can be helpful to save the compiled files as .rds files (R data files) so we don't have to re-compile the data
...
# we won't use these objects again, so we can remove them
rm(mtl_ufp_dat)
rm(corr_dat)
```
...
ggplot(mtl_ufp_dat_clean_std_long, aes(x = Value, y = ln_ufp)) + 
  facet_wrap(~Variable, scale="free_x") +
  geom_point(alpha=0.01) +
  geom_smooth(method="gam", col="blue") +
  geom_smooth(method="lm",col="red") +
  theme_classic()
# this will give some warning because for some variables you won't be able to fit a the non-linear function (the blue lines)

# we won't use these objects again, so we can remove them
rm(mtl_ufp_dat_clean_std_long)
...
# save the model performance as an .rds file
...
We can see that the linear models has R2 values in the external test sets of \~0.47 and slopes near to 1. In other applications, an R2 of 0.47 might be disappointing. It is not so bad for predicting spatial variations in outdoor UFPs, but we can likely do better. Past studies have reported R2 values between 0.30 and 0.80.
...
We can also map the model errors to look for potential spatial clustering of model residuals (which we want to avoid).
...
We can see the there might be some spatial clustering of model errors. We can also see that there are more instances of large under-predictions (red) than over-predictions.
...
# add the r2 to the model performance list
...
# we can look at performance in each of the CV iterations
...
#Perform a Random Search (100 iterations)
rand_search <- makeTuneControlRandom(maxit = 100)
...
xgb_CV
forest_CV # recall random forest model performance
```

How do the results from your XGBoost model compare to the other models? (mse.test.mean = ?, rsq.test.mean = ?).
...
#Add Residuals from XGBoost Model to mtl_ufp_dat_clean
...
NOTE: if you are using free or "Plus" Posit Cloud account, then you will not have access to enough memory to install the tensorflow package and train the dense neural network. Here is a post about other users running into that problem:
https://community.rstudio.com/t/tensorflow-can-not-install-in-rstudio-cloud/129441

However if you have a "Premium" or "Instructor" Posit Cloud account, you can click on the RAM icon in the top right and slide the memory allocated to 2 GB or higher. That will allow you to install tensorflow and train the dense neural network. If you are using R on your local machine, you likely have plenty of RAM and will also be able to install tensorflow and trina the dense neural network.
...
```{r}
# note, if the RAM for your session is running high and your session is crashing, it's because there are a lot of large packages and objects saved. the max allocated memory for the free and "Plus" Posit Cloud accounts is 1GB. 
# to free up memory, you can restart your R session (Session > Restart R) and clear your environment (broom icon in environment)
# if you did so, then we need to reload certain packages
...
surface_data <- 
  read_csv("data/surface_data/montreal_fishnet_landuse_sharp2023.csv") %>%
...
# the free Posit Cloud account might not be able to do this
plot_grid(ggdraw_lm, 
          ggdraw_gam, 
          ggdraw_forest, 
          ggdraw_xgboost, 
          # ggdraw_nn, # you can uncomment this line if you were able to train the neural network model
          ncol = 3)
...
model_r2_list <- readRDS("models/model_r2_list.rds")
model_r2_list
```
